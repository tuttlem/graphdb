...
fn scalar_function_math_numeric(input: &str) -> IResult<ScalarFunction> {
    alt((
        abs_function,
        ceil_function,
        floor_function,
...
fn scalar_function_math_logarithmic(input: &str) -> IResult<ScalarFunction> {
    alt((
        e_function,
        exp_function,
        log_function,
...
fn scalar_function_math_trigonometric(input: &str) -> IResult<ScalarFunction> {
    alt((
        acos_function,
        asin_function,
        atan2_function,
...
fn abs_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("abs"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Abs(expr)))
}

fn ceil_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("ceil"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Ceil(expr)))
}

fn floor_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("floor"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Floor(expr)))
}

fn isnan_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("isNaN"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::IsNaN(expr)))
}

fn rand_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("rand"))(input)?;
    let (input, _) = ws(char('('))(input)?;
    let (input, _) = ws(char(')'))(input)?;
    Ok((input, ScalarFunction::Rand))
}

fn round_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("round"))(input)?;
    let (input, _) = ws(char('('))(input)?;
    let (input, value) = non_aggregate_expression(input)?;
    let (input, precision_and_mode) = opt(|input| {
        let (input, _) = ws(char(','))(input)?;
        let (input, precision) = non_aggregate_expression(input)?;
        let (input, mode) = opt(|input| {
            let (input, _) = ws(char(','))(input)?;
            non_aggregate_expression(input)
        })(input)?;
        Ok((input, (precision, mode)))
    })(input)?;
    let (input, _) = ws(char(')'))(input)?;
    let (precision, mode) = precision_and_mode
        .map(|(precision, mode)| (Some(precision), mode))
        .unwrap_or((None, None));
    Ok((
        input,
...
fn sign_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("sign"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Sign(expr)))
}

fn e_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("e"))(input)?;
    let (input, _) = ws(char('('))(input)?;
    let (input, _) = ws(char(')'))(input)?;
    Ok((input, ScalarFunction::E))
}

fn exp_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("exp"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Exp(expr)))
}

fn log_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("log"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Log(expr)))
}

fn log10_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("log10"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Log10(expr)))
}

fn sqrt_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("sqrt"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Sqrt(expr)))
}

fn acos_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("acos"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Acos(expr)))
}

fn asin_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("asin"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Asin(expr)))
}

fn atan_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("atan"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Atan(expr)))
}

fn atan2_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("atan2"))(input)?;
    let (input, _) = ws(char('('))(input)?;
    let (input, y) = non_aggregate_expression(input)?;
    let (input, _) = ws(char(','))(input)?;
    let (input, x) = non_aggregate_expression(input)?;
    let (input, _) = ws(char(')'))(input)?;
    Ok((input, ScalarFunction::Atan2 { y, x }))
}

fn cos_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("cos"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Cos(expr)))
}

fn cot_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("cot"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Cot(expr)))
}

fn degrees_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("degrees"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Degrees(expr)))
}

fn haversin_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("haversin"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Haversin(expr)))
}

fn pi_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("pi"))(input)?;
    let (input, _) = ws(char('('))(input)?;
    let (input, _) = ws(char(')'))(input)?;
    Ok((input, ScalarFunction::Pi))
}

fn radians_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("radians"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Radians(expr)))
}

fn sin_function(input: &str) -> IResult<ScalarFunction> {
    let (input, _) = ws(tag_no_case("sin"))(input)?;
    let (input, expr) = delimited(ws(char('(')), non_aggregate_expression, ws(char(')')))(input)?;
    Ok((input, ScalarFunction::Sin(expr)))
}

fn tan_function(input: &str) -> IResult<ScalarFunction> {
